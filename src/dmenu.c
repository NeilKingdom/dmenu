...
static void drawhighlights(struct item *item, int x, int y, int maxw) {
    ...
    drw_setscheme(drw, scheme[item == sel ? SchemeSelHighlight :
        item->out ? SchemeOutHighlight : SchemeNormHighlight]);
    strcpy(tokens, text);
...
static void recalculatenumbers() {
    unsigned int numerator = 0, denomenator = 0;
    struct item *item;
    if (matchend) {
        numerator++;
    ...
    snprintf(numbers, NUMBERSBUFSIZE, "%d/%d", numerator, denomenator);
}
...
static void setup(void) {
    ...
    win = XCreateWindow(
        dpy, parentwin,
        x, y - (topbar ? 0 : border_width * 2),
        mw - border_width * 2, mh, border_width,
